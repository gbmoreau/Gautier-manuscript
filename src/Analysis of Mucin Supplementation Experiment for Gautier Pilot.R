...
# Sample reads have been processed using the DADA2 pipeline and organized into individual 
# experiments. For this analysis I'll focus on the Mucin Supplementation experiment, which 
# investigates the effect of mucin supplementation on the microbiome after stress treatment.
...
# other, with 4 weeks samples being distinct from the others. In addition, Control and Mucin-
...
# At the Phylum level, samples are dominated by members of the Bacteroidota and Firmicutes phyla. 
# There is a general trend of relative decreased abundance in Firmicutes and relative increased 
# abundance of Bacteroidota over the course of the experiment.
...
# Several Families are abundant across all groups, with the most abundant families including 
# Lachnospiraceae and Muribaculaceae. Of note, the Lactobacillus family was relatively increased at
# Baseline relative to all stressed conditions, which is consistent with previous results from the
# group. 

# I'll directly compare relative abundances of these families between groups of interest (Baseline vs.
# 3 Week Stress and Stress Alone vs. Stress + Mucin).
...
write.csv(Wilcox.Baseline.3wk, file = "./results/tables/mucin/Wilcoxon Rank Comparison_Baseline vs 3Wk Abundance.csv")
...
write.csv(Wilcox.Control.Mucin, file = "./results/tables/mucin/Wilcoxon Rank Comparison_Control vs Mucin Abundance.csv")
